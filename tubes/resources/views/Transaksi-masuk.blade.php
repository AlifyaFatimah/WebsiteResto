  ...
  <main>
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 side-bar" style="width: 280px;" >
      ...
      <div class="dropdown">
        ...
        <ul class="dropdown-menu text-small shadow" aria-labelledby="dropdownUser2">
          ...
          <li><a class="dropdown-item" href="/out">Sign out</a></li>
        </ul>
      </div>
    </div>
    <div class="container m-4" style="overflow: auto;">
      <ul class="nav nav-tabs justify-content-center nav-fill" id="myTab">
        ...
        <li class="nav-item">
          <a href="#masuk" class="nav-link active" data-bs-toggle="tab">Transaksi Masuk</a>
        </li>
        <!--Tab 2: Transaksi Berlangsung -->
        <li class="nav-item">
          <a href="{{URL::to('/Transaksi-berlangsung')}}" class="nav-link">Transaksi Berlangsung</a>
        </li>
        <!--Tab 3 : Riwayat Transaksi -->
        <li class="nav-item">
          <a href="{{URL::to('/Transaksi-riwayat')}}" class="nav-link">Riwayat</a>
        </li>
      </ul>
      <!--Content Tab 1: Transaksi Masuk -->
      <div class="tab-pane" id="masuk">
        <h4 class="my-4 text-semi">Transaksi Masuk</h4>
        <div class="card-content">
          @foreach ($masuk as $transaksi)
          <div class="card p-4 m-2">
            <p class="text-xs text-semi mb-1" id="idReservasi">IDORDER : {{$transaksi->idReservasi}}</p>
            <p class="text-2xs mb-3" id="date_create" style="color: #727472">{{$transaksi->created_at}}</p>
            <p class="text-2xs mb-1">{{$transaksi->idResto}} <span><svg class="bi bi-arrow-left-right mx-2" width="16" height="16"><use xlink:href="#Transaksi"/></svg></span> {{$transaksi->idCust}}</p>
            <p class="text-2xs mb-1" id="date" style="color: #727472">Tanggal Reservasi : {{$transaksi->date_reservasi}}</p>
            <p class="text-2xs mb-1" id="time" style="color: #727472">Waktu Reservasi : {{$transaksi->time_reservasi}}</p>
            <p class="text-2xs mb-4" id="no_table" style="color: #727472">No Meja : {{$transaksi->no_table}}</p>
            <span><a type="button" class="btn btn-primary" href="/Konfirmasi-masuk/{{$transaksi->idReservasi}}" onclick="return confirm('Yakin untuk menerima pesanan?')">Konfirmasi</a>
            <a type="button" class="btn btn-outline-danger" href="/Batal-masuk/{{$transaksi->idReservasi}}" onclick="return confirm('Yakin untuk membatalkan pesanan?')">Batal</a></span>
          </div>
          @endforeach
        </div>
      </div>
  ...
  <!-- <script>
    $(document).ready(function() {
        $('#Konfirmasi').click(function() {
          ...
          var idResto = $('#idResto').val();
          var idCust = $('#idCust').val();
          var date_reservasi = $('#date').val();
          var time_reservasi = $('#time').val();
          var no_table = $('#no_table').val();
          $.ajax({
              url: "/Konfirmasi-masuk",
              type: "POST",
              data: {
                  _token: $("#csrf").val(),
                  type: 1,
